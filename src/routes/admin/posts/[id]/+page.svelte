...
<div class="mb-8 flex items-center justify-between">
  <div>
    ...
    <h1 class="text-3xl font-bold font-['Playfair_Display'] text-[var(--text-primary)]">Editar Post</h1>
  </div>
</div>
...
<form method="POST" action="?/update" use:enhance class="bg-[var(--bg-card)] rounded-[var(--radius-lg)] shadow-[var(--shadow-sm)] border border-[var(--border)] p-6 md:p-8 space-y-6">
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
    ...
    <div class="space-y-2">
      <label for="slug" class="block text-sm font-medium text-[var(--text-secondary)]">Slug <span class="text-red-500">*</span></label>
      <input type="text" id="slug" name="slug" required value={data.post.slug}
        class="w-full rounded-[var(--radius-sm)] border border-[var(--border)] bg-transparent px-3 py-2 text-[var(--text-primary)] focus:border-[var(--accent)] focus:ring-1 focus:ring-[var(--accent)] outline-none transition-colors" />
    </div>
  </div>
  ...
  <div class="space-y-2">
    ...
    <textarea id="excerpt" name="excerpt" rows="2"
      class="w-full rounded-[var(--radius-sm)] border border-[var(--border)] bg-transparent px-3 py-2 text-[var(--text-primary)] focus:border-[var(--accent)] focus:ring-1 focus:ring-[var(--accent)] outline-none transition-colors resize-y">{data.post.excerpt ?? ''}</textarea>
  </div>

  <div class="space-y-2">
    <label for="cover_url" class="block text-sm font-medium text-[var(--text-secondary)]">URL de la Imagen de Portada</label>
    <input type="url" id="cover_url" name="cover_url" value={data.post.cover_url ?? ''}
      class="w-full rounded-[var(--radius-sm)] border border-[var(--border)] bg-transparent px-3 py-2 text-[var(--text-primary)] focus:border-[var(--accent)] focus:ring-1 focus:ring-[var(--accent)] outline-none transition-colors" />
  </div>
  ...
  <div class="space-y-2">
    ...
    <textarea id="content" name="content" rows="12" required
      class="w-full font-mono text-sm rounded-[var(--radius-sm)] border border-[var(--border)] bg-transparent px-3 py-2 text-[var(--text-primary)] focus:border-[var(--accent)] focus:ring-1 focus:ring-[var(--accent)] outline-none transition-colors resize-y">{data.post.content}</textarea>
  </div>

  <div class="flex items-center gap-3 pt-4 border-t border-[var(--border)]">
    <input type="checkbox" id="published" name="published" value="true" checked={data.post.published}
      class="w-4 h-4 rounded border-[var(--border)] text-[var(--accent)] focus:ring-[var(--accent)]" />
    <label for="published" class="text-sm font-medium text-[var(--text-primary)]">Publicado</label>
  </div>
